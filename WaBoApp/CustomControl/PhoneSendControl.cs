    ...
    public partial class PhoneSendControl : UserControl
    {
        ...
        private void btnUpload_Click(object sender, EventArgs e)
        {
            ...
            if (open.ShowDialog() == DialogResult.OK)
            {
                ...
                var list = ConvertUtil.ConvertList<PhoneModel>(dt);
                list = list.Select(x => new PhoneModel { Name = x.Name, Phone = x.Phone.TrimStart('+') }).ToList();
        ...
        private async void btnStart_Click(object sender, EventArgs e)
        {
            ...
            //this.AppApi.Refresh();
            var phoneList = this.dgvPhone.DataSource as List<PhoneModel>;
            var msgList = FileUtils.GetAll<MessageModel>().Where(m=>m.Type==MessageType.Import).ToList();
            if (msgList.Count == 0)
            {
        ...
        private void SendToUserList(List<PhoneModel> phoneList, List<MessageModel> msgList,
                ...
                try
                {
                    ...
                    else
                    {
                        if (!string.IsNullOrEmpty(msgModel.Content))
                        {
                            ...
                            result = this.AppApi.SendMsg(user.Phone, msg);
                        }
                        if (msgModel.IsAttachment && !string.IsNullOrEmpty(msgModel.Attachment))
                        {
                            ...
                            foreach (var item in fileList)
                            {
                                ...
                                {
                                    this.AppApi.SendFile(user.Phone, item);
                                }
                            }
        ...
        {
            var index = new Random().Next(msgList.Count-1);
        ...
        {
            var path = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Template", "importTemplate.xlsx");
