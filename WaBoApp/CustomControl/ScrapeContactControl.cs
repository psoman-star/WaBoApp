...
{
    public partial class ScrapeContactControl : UserControl
    {
        ...
        public override string Text => "Contact Send";
        public ScrapeContactControl()
        {
            InitializeComponent();
        }
        ...
        private void SendToUserList(List<string> phoneList, List<MessageModel> msgList,
            int msgNumDelay, int delay, int msgNum, CancellationToken token = default)
        {
            ...
            foreach (var phone in phoneList)
            {
                ...
                try
                {
                    ...
                    result = this.AppApi.SearchSendMsg(phone, msgModel.Content);
                    if (msgModel.IsAttachment && !string.IsNullOrEmpty(msgModel.Attachment))
                    {
                        ...
                        foreach (var item in fileList)
                        {
                            ...
                            {
                                this.AppApi.SendFile(phone, item);
                            }
                        }
                    }
        ...
        private void btnContact_Click(object sender, EventArgs e)
        {
            ...
            {
                this.AppApi.GetContactList(action);
            }).ContinueWith(t =>
            {
                this.Invoke(new Action(() =>
                {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            var list=new List<string>();
            foreach (DataGridViewRow row in this.dgvPhone.Rows)
            {
            ...
            {
                Filter = "*.txt|txt File",
                FileName = $"ContactList-{Environment.TickCount}.txt"
            };
            if (open.ShowDialog() == DialogResult.OK)
            {
                File.WriteAllLines(open.FileName, list);
                KryptonMessageBox.Show("Save Success!", "Info",
                          MessageBoxButtons.OK, MessageBoxIcon.Information, showCtrlCopy: false);
            }
